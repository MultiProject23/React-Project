...
const ReviewListItem = ({product, content, account, image, openModal}) => {
  ...
  const truncatedContent = content.length > MAX_CONTENT_LENGTH ? content.substr(0, MAX_CONTENT_LENGTH) + '...' : content; //100글자 넘으면 ... 으로 표시.
  return (
    <S.Review_list_item>
      ...
      <S.Review_list_item_detail2>
        ...
        <S.Review_list_item_ContentWrite onClick={onClick}>{truncatedContent}</S.Review_list_item_ContentWrite>
